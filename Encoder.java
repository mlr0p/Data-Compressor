...
class Encoder {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// Check 2^dictionaryCapacity will be greater than the initial number of phrases
			// in the trie
			...
			// Counter variable to store the maximum phrase number currently in trie,
			// initialised to 255
			int maxPhraseNum = 255;
			/*
			 * Repeatedly find a phrase number for a matching prefix, output the phrase
			 * number and add the first mismatch character with the next available phrase
			 * number to the trie
			 */
			while (input_bytes.length != 0) {
				...
				if (input_bytes.length != 0) {
					...
					if (maxPhraseNum >= dictionaryCapacity) {
						// Throw away all acquired phrases and continue encoding as if starting from
						// scratch
